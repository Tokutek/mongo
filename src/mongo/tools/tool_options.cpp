...
namespace mongo {
    ...
    Status addLocalServerToolOptions(moe::OptionSection* options) {
        ...
        ret = options->addOption(OD("journal", "journal", moe::Switch,
                    "enable journaling (relevant only if dbpath specified)", false));
        if(!ret.isOK()) {
            return ret;
        }
    ...
    Status addMongoRestoreOptions(moe::OptionSection* options) {
        ...
        ret = options->addOption(OD("w", "w", moe::Int ,
                    ...
                    " WARNING: setting w > 1 prevents the bulk load optimization", true, moe::Value(0)));
        if(!ret.isOK()) {
            return ret;
        }
        ret = options->addOption(OD("noLoader", "noLoader", moe::Switch,
                    "don't use bulk loader", true, moe::Value(true)));
        if(!ret.isOK()) {
            return ret;
        }
        ...
        ret = options->addOption(OD("pageSize", "pageSize", moe::Int ,
