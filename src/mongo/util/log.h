...
namespace logger {
    ...
    /**
     * Returns a LogstreamBuilder for logging a message with [tokumx] prepended.
     * (sort of legacy)
     */
    inline LogstreamBuilder tokulog() {
        return LogstreamBuilder(logger::globalLogDomain(),
                                "tokumx",
                                logger::LogSeverity::Log());
    }
...
#define MONGO_TOKULOG(DLEVEL) \
    if (!(::mongo::logger::globalLogDomain())->shouldLog(::mongo::LogstreamBuilder::severityCast(DLEVEL))) {} \
    else LogstreamBuilder(::mongo::logger::globalLogDomain(), "tokumx", ::mongo::LogstreamBuilder::severityCast(DLEVEL))
