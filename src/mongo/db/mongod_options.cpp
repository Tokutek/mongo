...
namespace mongo {
    ...
    Status addMongodOptions(moe::OptionSection* options) {
        ...
        ret = general_options.addOption(OD("gdb", "gdb", moe::Switch,
                    "go into a gdb-friendly mode (development use only)", true));
        if (!ret.isOK()) {
            return ret;
        }
        ret = general_options.addOption(OD("gdbPath", "gdbPath", moe::String,
        ...
        ret = replication_options.addOption(OD("txnMemLimit", "txnMemLimit", moe::UnsignedLongLong,
                    "limit of the size of a transaction's operation", true, moe::Value(1ULL<<20)));
        if (!ret.isOK()) {
            return ret;
        }
        ...
        ret = options->addOption(OD("nssize", "nssize", moe::Int,
                    ".ns file size (in MB) for new databases", false, moe::Value(16)));
        if (!ret.isOK()) {
            return ret;
        }
