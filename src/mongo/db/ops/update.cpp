...
namespace mongo {
    ...
    void updateOneObject(NamespaceDetails *d, const BSONObj &pk, const BSONObj &oldObj, const BSONObj &newObj) {
        dassert( !newObj["_id"].eoo() && newObj["_id"] == oldObj["_id"] );

        tokulog(1) << "updateOneObject: del pk " << pk << ", obj " << oldObj << " and inserting " << newObj << endl;
        deleteOneObject( d, pk, oldObj );
    ...
    static void updateUsingMods(NamespaceDetails *d, const BSONObj &pk, const BSONObj &obj, ModSetState &mss) {
        ...
        updateOneObject( d, pk, obj, newObj );
    }
    ...
    static void updateNoMods(NamespaceDetails *d, const BSONObj &pk, const BSONObj &obj, const BSONObj &updateobj) {
        ...
        if ( updateobj["_id"].eoo() ) {
            ...
            updateOneObject( d, pk, obj, b.done() );
        } else {
            updateOneObject( d, pk, obj, updateobj );
        }
    }

    static void insertAndLog(const char *ns, NamespaceDetails *d, const BSONObj &newObj, bool logop, bool fromMigrate) {
        checkNoMods( newObj );
        tokulog(1) << "insertAndLog for upsert: " << newObj << endl;

        // Make sure to add an _id field if it doesn't exist.
        insertOneObject( d, newObj["_id"].eoo() ? addIdField(newObj) : newObj );
        if ( logop ) {
    ...
    */
    static UpdateResult _updateById(const BSONObj &pk,
        ...
        updateNoMods(d, pk, obj, updateobj);
    ...
    UpdateResult _updateObjects( bool su,
        ...
        if( c->ok() ) {
            ...
            do {
                ...
                if ( isOperatorUpdate ) {
                    ...
                    updateUsingMods( d, currPK, currentObj, *mss );
                ...
                updateNoMods( d, currPK, currentObj, updateobj );
        ...
        if ( upsert ) {
            ...
            debug.upsert = true;
            insertAndLog(ns, d, updateobj, logop, fromMigrate);
            return UpdateResult( 0 , 0 , 1 , updateobj );
        }
