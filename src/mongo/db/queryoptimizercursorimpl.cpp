...
namespace mongo {
    ...
    public:
        ...
        bool currentMatches( MatchDetails *details ) {
            ...
            if ( _explainPlanInfo ) {
                // Note iterate results as if this is the only query plan running.  But do not
                // account for query parameters that may be appled to the whole result set (results
                // from interleaved plans), for example the 'skip' parameter.
