...
namespace mongo {
    ...
    class HashedDescriptor : public Descriptor {
    public:
        // For creating a brand new descriptor.
        HashedDescriptor(const BSONObj &keyPattern,
                         const int hashSeed,
                         const int hashVersion,
                         const bool sparse = false,
                         const bool clustering = false) :
            Descriptor(keyPattern, Descriptor::Hashed, sparse, clustering),
            _hashSeed(hashSeed),
            _hashVersion(hashVersion) {
        }
        virtual ~HashedDescriptor() { }

        // For interpretting a memory buffer as a descriptor.
        HashedDescriptor(const char *data, const size_t size);
