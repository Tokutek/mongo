...
#endif
        ret = options->addOption(OD("gdb", "gdb", moe::Switch,
                    "go into a gdb-friendly mode (development use only)", true));
        if (!ret.isOK()) {
            return ret;
        }
        ret = options->addOption(OD("gdbPath", "gdbPath", moe::String,
                    "if specified, debugging information will be gathered on fatal error by launching gdb at the given path", true));
        if (!ret.isOK()) {
            return ret;
        }
        ret = options->addOption(OD("pluginsDir", "pluginsDir", moe::String,
                    "directory containing plugins (defaults to lib64/plugins)", true));
        if (!ret.isOK()) {
            return ret;
        }
        ret = options->addOption(OD("loadPlugin", "loadPlugin", moe::StringVector,
                    "load plugins at startup", true));
        if (!ret.isOK()) {
            return ret;
        }
