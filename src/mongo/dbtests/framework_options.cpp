...
namespace mongo {
    ...
    Status addTestFrameworkOptions(moe::OptionSection* options) {
        ...
        ret = options->addOption(OD("bigfiles", "bigfiles", moe::Switch,
                    "use big datafiles instead of smallfiles which is the default", false));
        if (!ret.isOK()) {
            return ret;
        }
        ...
        ret = options->addOption(OD("dur", "dur", moe::Switch,
                    "enable journaling (currently the default)", false));
        if (!ret.isOK()) {
            return ret;
        }
    ...
    Status storeTestFrameworkOptions(const moe::Environment& params,
        ...
        if (params.count("useNewQueryFramework")) {
            std::cerr << "useNewQueryFramework is not yet supported" << std::endl;
        ...
        if( params.count("nodur") ) {
            std::cerr << "nodur is deprecated" << std::endl;
            ::_exit(EXIT_BADOPTIONS);
        }
        if( params.count("dur") /* || storageGlobalParams.dur */ ) {
            std::cerr << "dur is deprecated" << std::endl;
            ::_exit(EXIT_BADOPTIONS);
        }
        ...
        if( params.count("bigfiles") ) {
            std::cerr << "bigfiles is deprecated" << std::endl;
            ::_exit(EXIT_BADOPTIONS);
        }
