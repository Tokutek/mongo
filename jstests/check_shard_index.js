...
res = db.runCommand( { checkShardingIndex: db.getName() + ".jstests_shardingindex" , keyPattern: {x:1, y:1} });
...
// Check _id index
res = db.runCommand( { checkShardingIndex: db.getName() + ".jstests_shardingindex" , keyPattern: {_id:1} });
...
f.remove( { y : 2 } );
// tokuds doesn't recalculate multikeyindexbits during reindex, must drop and re-create index explicitly
f.dropIndex({x:1,y:1});
f.ensureIndex({x:1,y:1});
//f.reIndex();
...
res = db.runCommand( { checkShardingIndex: db.getName() + ".jstests_shardingindex" , keyPattern: {x:1} });
...
res = db.runCommand( { checkShardingIndex: db.getName() + ".jstests_shardingindex" , keyPattern: {x:1, y:1, z:1} });
