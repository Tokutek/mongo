...
assert.eq(1, db.system.indexes.find({ "ns" : "test.rollbackdropindex", "key" : { a : 1 } }).count());
...
assert(!db.getLastError());
assert.eq(2, db.system.namespaces.find({ "name" : { $regex: "rollbackdropindex" } }).count());
assert.eq(0, db.system.indexes.find({ "ns" : "test.rollbackdropindex", "key" : { a : 1 } }).count())
...
assert.eq(1, db.system.indexes.find({ "ns" : "test.rollbackdropindex", "key" : { b : 1 } }).count());
...
assert.eq(2, db.system.namespaces.find({ "name" : { $regex: "rollbackdropindex" } }).count());
assert.eq(0, db.system.indexes.find({ "ns" : "test.rollbackdropindex", "key" : { a : 1 } }).count())
assert.eq(0, db.system.indexes.find({ "ns" : "test.rollbackdropindex", "key" : { b : 1 } }).count())
assert.eq(1, t.stats().nindexes)
assert.throws(t.find({ a: 50 }).hint({ a: 1 })) // "bad hint"
