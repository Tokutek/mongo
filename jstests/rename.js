...
c.save( {a: 100} );
assert.commandFailed( admin.runCommand( {renameCollection:db.getName() + ".jstests_rename_a", to:db.getName() + ".jstests_rename_c"} ) );

assert.commandWorked( admin.runCommand( {renameCollection:db.getName() + ".jstests_rename_a", to:db.getName() + ".jstests_rename_b"} ) );
assert.eq( 0, a.find().count() );

assert.eq( 2, b.find().count() );
assert( db.system.namespaces.findOne( {name:db.getName() + ".jstests_rename_b" } ) );
assert( !db.system.namespaces.findOne( {name:db.getName() + ".jstests_rename_a" } ) );
assert.eq( 3, db.system.indexes.find( {ns:db.getName() + ".jstests_rename_b"} ).count() );
assert( b.find( {a:1} ).explain().cursor.match( /^IndexCursor/ ) );
...
assert( db.system.namespaces.findOne( {name:db.getName() + ".jstests_rename_b" } ) );
assert( !db.system.namespaces.findOne( {name:db.getName() + ".jstests_rename_a" } ) );
assert.eq( true, db.system.namespaces.findOne( {name:db.getName() + ".jstests_rename_b"} ).options.capped );
