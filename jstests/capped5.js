...
assert.eq( 2, db.system.indexes.count( {ns:db.getName() + "."+tn} ) ); //now we assume _id index (tokudb: 2 keys, $_ and _id)
...
t.insert( { _id : 5 , x : 11 } );
t.insert( { _id : 5 , x : 12 } );
assert.eq( 2, db.system.indexes.count( {ns:db.getName() + "."+tn} ) ); //now we assume _id index
...
assert.eq( 3, db.system.indexes.count( {ns:db.getName() + "."+tn} ) ); //now we assume _id index (tokudb: 3 keys, $_ and _id and x)
