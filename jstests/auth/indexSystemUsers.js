...
testDB.exploit.system.indexes.insert({ns: db.getName() + ".system.users", key: { haxx: 1.0 }, name: "haxx_1",
...
assert.eq(null,
          testDB.system.namespaces.findOne(
              {$and : [{name : /^test\.system\.users\.\$/},
                       {name : {$ne : db.getName() + ".system.users.$_id_"}},
                       {name : {$ne : db.getName() + ".system.users.$user_1_userSource_1"}} ]}));
adminDB.logout();
