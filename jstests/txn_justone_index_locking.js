...
assert(!db.getLastError());
s1 = startParallelShell('db.txnjustonelocking.remove({ _id: { $gte: 2 } }, { justOne: true }); assert(!db.getLastError()); db.txnjustonelocking.insert({ success: "deleteJustOne2" })');
s1();
commit();
...
assert(!db.getLastError());
s1 = startParallelShell('db.txnjustonelocking.update({ _id: { $gte: 2 } }, { $set: { c: 1 } }, { multi: false }); assert(!db.getLastError()); db.txnjustonelocking.insert({ success: "updateMultiFalse2" })');
s1();
commit();
...
assert(!db.getLastError());
s1 = startParallelShell('db.txnjustonelocking.findAndModify({ query: { _id: { $gte: 2 } }, update: { $set: { c: 1 } }}); assert(!db.getLastError()); db.txnjustonelocking.insert({ success: "findAndModify2" })');
s1();
commit();
