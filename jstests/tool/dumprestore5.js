...
assert.eq(2, db.system.indexes.count(), "setup2") // namespaces($_) + user(_id)
...
assert.eq(2, db.system.indexes.count(), "didn't restore indexes") // namespaces($_) + user(_id)
...
assert.eq(2, db.system.indexes.count(), "didn't maintain indexes") // namespaces($_) + user(_id)
