...
assert.eq(3, db.system.indexes.count(), "setup2") // namespaces($_) + user(_id) + compound user index
...
assert.eq(3, db.system.indexes.count(), "didn't restore indexes") // namespaces($_) + user(_id) + compound user index
...
assert.eq(3, db.system.indexes.count(), "didn't maintain indexes") // namespaces($_) + user(_id) + compound user index
